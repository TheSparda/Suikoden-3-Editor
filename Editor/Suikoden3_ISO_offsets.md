...
**CONFIRMED IN PLAY 2026-09-06** — the stranded bit soft-locked the battle; with `syncNoAim`
moving it, Phoenix retargeted to All foes offers the enemy side and resolves against all foes.
...
which recomputes bit 16 from the final target byte and area bit — and the same Phoenix edit, redone
on a fixed build, was played and behaves. That is what promotes this from a clean 132/132
correlation to a mechanism: the bit was moved deliberately and the battle changed accordingly.

Not claimed: the unite side. Same field, same function, but no unite has been retargeted and
played, and a play report does not transfer across patch shapes. **Any tool that writes this field
