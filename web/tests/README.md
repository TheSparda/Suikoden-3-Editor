...
- **Only some builds work, and it is per platform.** On macOS `chromium_headless_shell-1148`
  passes the suite while `chromium_headless_shell-1243` **aborts in the same section as Chrome
  for Testing**; on Linux, `1234` runs the whole suite green in CI. So "newest headless_shell"
  is not the rule, and the module keeps `VERIFIED_BY_PLATFORM` — an explicit list of build
  revisions keyed by `process.platform`, not a pattern. It is keyed by platform because a bare
  revision list would either cry wolf on every green CI run or vouch for a macOS build nobody
  has run. Adding an entry means running the *whole* suite on that platform first, and saying
  in the comment where the evidence came from.
